...
/*
vec4 calculate2DNoiseDisplacementVector( vec4 tex1) {
    float noiseScale = 100;
    float noiseStrength = 200;
    float scale = 100;
    
    vec2 noiseVecX = vec2( tex1.r/noiseScale, tex1.a/noiseScale);
    float displacementX = cos(cnoise( noiseVecX ) * noiseStrength) * scale;
    
    vec2 noiseVecY = vec2( tex1.g / noiseScale, tex1.a / noiseScale);
    float displacementY = sin( cnoise( noiseVecY) * noiseStrength ) * scale;
    
    vec2 noiseVecZ = noiseVecY / noiseVecX;
